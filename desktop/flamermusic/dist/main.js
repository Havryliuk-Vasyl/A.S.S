...
const createWindow = () => {
  ...
  ipcMain.on('show-context-menu', (event, context) => {
    const contextMenu = Menu.buildFromTemplate(getContextMenuTemplate(context));
...
function getContextMenuTemplate(context) {
  const menuTemplate = [{
    label: 'Дія 1',
    click: () => {
      console.log('Виконано дію 1 з контекстом:', context);
    }
  }, {
    label: 'Дія 2',
    click: () => {
      console.log('Виконано дію 2 з контекстом:', context);
    }
  }, {
    type: 'separator'
  }, {
  ...
  if (context.type === 'item') {
    menuTemplate.push({
      label: 'Додаткова дія для елемента',
      click: () => {
        console.log('Виконано додаткову дію для елемента');
      }
    });
  } else if (context.type === 'folder') {
    menuTemplate.push({
      label: 'Додаткова дія для папки',
      click: () => {
        console.log('Виконано додаткову дію для папки');
      }
    });
  }
  return menuTemplate;
